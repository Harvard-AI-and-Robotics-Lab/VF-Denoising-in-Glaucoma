...
def create_circular_mask(h, w, center=None, radius=None):
    ...
    if radius is None: radius = int(min(h, w) * 0.15)
    Y, X = np.ogrid[:h, :w]
...
class VF2RNFL_Model(nn.Module):
    def __init__(self, decoder, z_dim=32, dropout=0.2, img_size=200, skip_channels=16):
        ...
        # NEW: Skip connection module
        self.vf_skip = VF_Skip_Module(img_size=img_size, skip_channels=skip_channels)
        
        # NEW: Fusion layer to combine decoder output with VF skip features
...
def train(args):
    ...
    with torch.no_grad():
        for _, rnflt in tqdm(train_loader):
            z = ae.encoder(rnflt.to(device).unsqueeze(1))
            z_list.append(z)
    ...
    for p in model.vf_enc.parameters(): p.requires_grad = True
    
    # NEW: Always unfreeze skip connection and fusion modules (they are new!)
    for p in model.vf_skip.parameters(): p.requires_grad = True
    for p in model.fusion_layer.parameters(): p.requires_grad = True
    ...
    if args.freeze_strategy == "all":
        ...
        for p in model.decoder.up_final.parameters(): p.requires_grad = True
    elif args.freeze_strategy == "half":
        print("   -> Decoder: Unfreezing 'up4' and 'up_final'")
        for p in model.decoder.up4.parameters(): p.requires_grad = True
        for p in model.decoder.up_final.parameters(): p.requires_grad = True
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for vf, rnflt in tqdm(train_loader, desc=f"Ep {epoch}"):
            vf, rnflt = vf.to(device), rnflt.to(device).unsqueeze(1)
...
if __name__ == "__main__":
    ...
    # NEW: Skip connection channels
    parser.add_argument("--skip_channels", type=int, default=16, help="Number of channels in VF skip connection")
    
    parser.add_argument("--freeze_strategy", type=str, default="all", choices=["all", "tail", "half"])
    parser.add_argument("--split_ratio", type=float, default=0.2)
    parser.add_argument("--split_seed", type=int, default=42)
    parser.add_argument("--project", type=str, default="VF_to_RNFL_Spatial")
    parser.add_argument("--run_name", type=str, default="vf_skip_v1")
...
"""
# RECOMMENDED USAGE FOR DENOISED DATA:

# Version 1: Conservative (frozen decoder, rely on skip connections)
nohup python vf_to_rnfl_skip.py \
  --folder $VF_DATA_ROOT/original_below350 \
  --autoencoder_ckpt checkpoints/masked30_ae_z32_autoencoder_best.pth \
  --run_name Skip16_Raw_Tail \
  --z_dim 32 \
  --lr 3e-4 \
  --pixel_lambda 8.0 \
  --latent_lambda 1.0 \
  ...
  > logs/skip_raw.log 2>&1 &

# Version 2: Adaptive (allow decoder tail to fine-tune)
nohup python vf_to_rnfl_final.py \
  --folder $VF_DATA_ROOT/denoised_NAFNet_models/denoised_NAFNet_Advanced \
  --autoencoder_ckpt checkpoints/masked30_ae_z32_autoencoder_best.pth \
  --run_name NAFNet_Skip24_Tail \
  --z_dim 32 \
  --lr 3e-4 \
  --pixel_lambda 10.0 \
  --latent_lambda 0.5 \
  ...
  --skip_channels 24 \
  --freeze_strategy tail \
  > NAFNet_Skip_v2.log 2>&1 &

# Version 3: Max capacity (more skip channels, half decoder unfrozen)
nohup python vf_to_rnfl_final.py \
  --folder $VF_DATA_ROOT/denoised_NAFNet_models/denoised_NAFNet_Advanced \
  --autoencoder_ckpt checkpoints/masked30_ae_z32_autoencoder_best.pth \
  --run_name NAFNet_Skip32_Half \
  --z_dim 32 \
  --lr 1e-4 \
  --pixel_lambda 10.0 \
  --latent_lambda 1.0 \
