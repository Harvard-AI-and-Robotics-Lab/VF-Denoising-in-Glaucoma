"""
Test Script with Unified Severity Mapping for AvgVF Model

Uses pre-computed severity mapping from raw data,
ensuring consistent severity grouping across all denoising methods.

Key difference from test_unified_severity.py:
  - VF input is averaged (replaced with uniform mean) before feeding to model,
    matching the training-time behavior of vf_to_rnfl_skip_avg.py.
"""
...
# ============================================================================
# 3. TEST DATASET WITH SEVERITY MAPPING (AvgVF version)
# ============================================================================
...
class TestDatasetWithSeverity(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # AvgVF: Replace spatial VF with uniform mean value
        ...
        return vf_avg, rnfl_tensor, filename, severity_info
...
def main(args):
    ...
    # Build model
    print(f"\nBuilding AvgVF model with {args.skip_channels}-channel skip connections...")
    ...
    if 'model_state_dict' in ckpt:
        ...
        if 'val_mae' in ckpt:
            print(f"   Val MAE: {ckpt['val_mae']:.2f} um")
        if 'val_r2' in ckpt:
            print(f"   Val R2: {ckpt['val_r2']:.4f}")
    else:
    ...
    print(f"\nRunning Inference (AvgVF baseline)...")
    ...
    with torch.no_grad():
        for vf_avg, gt, filenames, severity_info_batch in tqdm(loader, desc="Evaluating"):
            vf_avg = vf_avg.to(device)

            # Predict (model returns tuple: prediction, z_norm)
            pred_norm, _ = model(vf_avg)
            pred_norm = pred_norm.cpu().squeeze(1).numpy()
    ...
    # ================= REPORTING =================
    print("\n" + "="*90)
    print(f"FINAL RESULTS (AvgVF Baseline): {os.path.basename(args.checkpoint)}")
    print(f"Dataset: {Path(args.test_folder).name}")
    print(f"Skip Channels: {args.skip_channels}")
    print(f"Severity based on: {os.path.basename(args.severity_mapping)}")
    print("="*90)
    print(f"{'GROUP':<15} | {'N_SAMPLES':<10} | {'MAE (um)':<20} | {'AVG R2':<10} | {'GLOBAL R2':<10}")
    print("-" * 90)
    ...
    if len(all_metrics["mae"]) > 0:
        ...
        print(f"   Min MAE:    {np.min(all_metrics['mae']):.2f} um")
        print(f"   Max MAE:    {np.max(all_metrics['mae']):.2f} um")
        print(f"   Median MAE: {np.median(all_metrics['mae']):.2f} um")
        print(f"   25th %ile:  {np.percentile(all_metrics['mae'], 25):.2f} um")
        print(f"   75th %ile:  {np.percentile(all_metrics['mae'], 75):.2f} um")

    print()

if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Test AvgVF->RNFL with unified severity mapping"
    )
    parser.add_argument(
        "--test_folder",
        type=str,
...
# Option A: Test folder contains ONLY test samples
python test_unified_severity_avg.py \
  --test_folder /path/to/test/folder \
  --checkpoint checkpoints/AvgVF_Raw_Frozen_best.pth \
  --severity_mapping splits/severity_mapping.json \
  --skip_channels 16

# Option B: Test folder contains ALL samples (train+val+test mixed)
python test_unified_severity_avg.py \
  --test_folder $VF_DATA_ROOT/original_below350 \
  --checkpoint checkpoints/AvgVF_Raw_Frozen_best.pth \
  --severity_mapping splits/severity_mapping.json \
  --skip_channels 16 \
  --use_split_filter

# Test all AvgVF models with consistent severity grouping
DATASETS=(
  "original_below350"
  "denoised_n2n_FINAL"
...
for dataset in "${DATASETS[@]}"; do
  ...
  echo "Testing AvgVF: $dataset_name"

  python test_unified_severity_avg.py \
    --test_folder $VF_DATA_ROOT/$dataset \
    --checkpoint checkpoints/AvgVF_${dataset_name}_best.pth \
    --severity_mapping splits/severity_mapping.json \
    --skip_channels 16 \
    --use_split_filter
done
